package com.google.android.gms.internal.ads;

import android.os.RemoteException;
/* compiled from: com.google.android.gms:play-services-ads@@23.1.0 */
/* loaded from: classes3.dex */
public final class zzeif extends zzbqt implements zzdaa {
    private zzbqu zza;
    private zzczz zzb;

    @Override // com.google.android.gms.internal.ads.zzdaa
    public final synchronized void zza(zzczz zzczzVar) {
        this.zzb = zzczzVar;
    }

    public final synchronized void zzc(zzbqu zzbquVar) {
        this.zza = zzbquVar;
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zze() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zze();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzf() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzf();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzg(int i) throws RemoteException {
        zzczz zzczzVar = this.zzb;
        if (zzczzVar != null) {
            zzczzVar.zza(i);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzh(com.google.android.gms.ads.internal.client.zze zzeVar) throws RemoteException {
        zzczz zzczzVar = this.zzb;
        if (zzczzVar != null) {
            zzczzVar.zzb(zzeVar);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzi(int i, String str) throws RemoteException {
        zzczz zzczzVar = this.zzb;
        if (zzczzVar != null) {
            zzczzVar.zzc(i, str);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzj(int i) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzj(i);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzk(com.google.android.gms.ads.internal.client.zze zzeVar) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzk(zzeVar);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzl(String str) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzl(str);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzm() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzm();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzn() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzn();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzo() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzo();
        }
        zzczz zzczzVar = this.zzb;
        if (zzczzVar != null) {
            zzczzVar.zzd();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzp() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzp();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzq(String str, String str2) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzq(str, str2);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzr(zzbhy zzbhyVar, String str) throws RemoteException {
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzs(zzbyc zzbycVar) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzs(zzbycVar);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzt(zzbyg zzbygVar) throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzt(zzbygVar);
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzu() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzu();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzv() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzv();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzw() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzw();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzx() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzx();
        }
    }

    @Override // com.google.android.gms.internal.ads.zzbqu
    public final synchronized void zzy() throws RemoteException {
        zzbqu zzbquVar = this.zza;
        if (zzbquVar != null) {
            zzbquVar.zzy();
        }
    }
}
