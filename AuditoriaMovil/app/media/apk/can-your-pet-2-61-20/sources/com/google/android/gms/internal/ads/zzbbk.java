...
public final class zzbbk {
    ...
    public zzbbk(int i, int i2, int i3) {
        this.zzb = i;
        i2 = (i2 > 64 || i2 < 0) ? 64 : 64;
        if (i3 <= 0) {
            this.zzd = 1;
        ...
        this.zza = new zzbbi(i2);
    }

    public final String zza(ArrayList arrayList, ArrayList arrayList2) {
        Collections.sort(arrayList2, new zzbbj(this));
        HashSet hashSet = new HashSet();
        loop0: for (int i = 0; i < arrayList2.size(); i++) {
            String[] split = Normalizer.normalize((CharSequence) arrayList.get(((zzbay) arrayList2.get(i)).zze()), Normalizer.Form.NFKC).toLowerCase(Locale.US).split("\n");
            if (split.length != 0) {
        ...
        while (it.hasNext()) {
            try {
                zzbbbVar.zzb.write(this.zza.zzb((String) it.next()));
            } catch (IOException e) {
