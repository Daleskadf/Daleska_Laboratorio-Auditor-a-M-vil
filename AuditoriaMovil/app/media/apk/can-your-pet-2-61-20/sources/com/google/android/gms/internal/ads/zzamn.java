package com.google.android.gms.internal.ads;

import java.util.ArrayList;
import java.util.Collections;
/* compiled from: com.google.android.gms:play-services-ads@@23.1.0 */
/* loaded from: classes3.dex */
public final class zzamn implements zzalh {
    private final zzfo zza = new zzfo();

    @Override // com.google.android.gms.internal.ads.zzalh
    public final void zza(byte[] bArr, int i, int i2, zzalg zzalgVar, zzep zzepVar) {
        zzec zzp;
        this.zza.zzI(bArr, i2 + i);
        this.zza.zzK(i);
        ArrayList arrayList = new ArrayList();
        while (true) {
            zzfo zzfoVar = this.zza;
            if (zzfoVar.zzb() > 0) {
                zzek.zze(zzfoVar.zzb() >= 8, "Incomplete Mp4Webvtt Top Level box header found.");
                zzfo zzfoVar2 = this.zza;
                int zzg = zzfoVar2.zzg() - 8;
                if (zzfoVar2.zzg() == 1987343459) {
                    zzfo zzfoVar3 = this.zza;
                    CharSequence charSequence = null;
                    zzea zzeaVar = null;
                    while (zzg > 0) {
                        zzek.zze(zzg >= 8, "Incomplete vtt cue box header found.");
                        int zzg2 = zzfoVar3.zzg();
                        int zzg3 = zzfoVar3.zzg();
                        int i3 = zzg - 8;
                        int i4 = zzg2 - 8;
                        String zzB = zzfx.zzB(zzfoVar3.zzM(), zzfoVar3.zzd(), i4);
                        zzfoVar3.zzL(i4);
                        if (zzg3 == 1937011815) {
                            zzeaVar = zzamx.zzb(zzB);
                        } else if (zzg3 == 1885436268) {
                            charSequence = zzamx.zza(null, zzB.trim(), Collections.emptyList());
                        }
                        zzg = i3 - i4;
                    }
                    if (charSequence == null) {
                        charSequence = "";
                    }
                    if (zzeaVar != null) {
                        zzeaVar.zzl(charSequence);
                        zzp = zzeaVar.zzp();
                    } else {
                        zzamw zzamwVar = new zzamw();
                        zzamwVar.zzc = charSequence;
                        zzp = zzamwVar.zza().zzp();
                    }
                    arrayList.add(zzp);
                } else {
                    this.zza.zzL(zzg);
                }
            } else {
                zzepVar.zza(new zzakz(arrayList, -9223372036854775807L, -9223372036854775807L));
                return;
            }
        }
    }
}
