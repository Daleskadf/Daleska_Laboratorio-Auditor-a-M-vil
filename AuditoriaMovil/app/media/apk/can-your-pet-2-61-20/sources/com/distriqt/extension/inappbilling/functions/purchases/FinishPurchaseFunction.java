    ...
    public FREObject call(FREContext fREContext, FREObject[] fREObjectArr) {
        try {
            ...
            if (inAppBillingContext.v) {
                ...
                purchase.cancelTimestamp = (long) fREObjectArr[0].getProperty("cancelTimestamp").getAsDouble();
                bool = Boolean.valueOf(inAppBillingContext.controller().service().finishPurchase(purchase));
            }
            return FREObject.newObject(bool.booleanValue());
        } catch (Exception e) {
            Errors.handleException(fREContext, e);
            ((InAppBillingContext) fREContext).dispatchEvent(InAppBillingEvent.FINISH_FAILED, PurchaseEvent.formatErrorForEvent(5, "Invalid purchase details provided"));
            return null;
        }
    }
}
