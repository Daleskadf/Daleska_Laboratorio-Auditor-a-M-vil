...
public class GetAdSizeFunction implements FREFunction {
    @Override // com.adobe.fre.FREFunction
    public FREObject call(FREContext fREContext, FREObject[] fREObjectArr) {
        try {
            AdvertsContext advertsContext = (AdvertsContext) fREContext;
            if (advertsContext.v) {
                AdvertView adView = advertsContext.controller().platform().getAdView(fREObjectArr[0].getAsString());
                if (adView != null) {
                    return FREObject.newObject(adView.getAdSize().toJSON().toString());
                }
                return null;
            }
            return null;
