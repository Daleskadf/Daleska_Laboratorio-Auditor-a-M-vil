...
class DeviceProfiling {
    ...
    public static boolean checkAndInitiateProfiler(Activity activity) {
        ...
        boolean resExists2 = resourceFileManager2.resExists(resourceFileManager2.lookupResId("raw.profileragent"));
    ...
    private static void insertTrustConfigEntry() {
        ...
        try {
            Utils.writeStringToFile(sb2, new File(ApplicationFileManager.getAppRoot() + File.separator + str + File.separator + FLASH_TRUST_DIR + File.separator + PROFILE_CONFIG_FILENAME));
        } catch (Exception e2) {
    ...
    private static void insertPreloadTagEntry() {
        File file = new File(Utils.GetExternalStorageDirectory() + File.separator + ".adobe" + File.separator + AndroidConstants.AIR + File.separator + MM_DOT_CFG);
        ...
        if (hashMap.get(HttpHeaders.HOST) != null) {
            concat = concat.concat("host=").concat(hashMap.get(HttpHeaders.HOST));
        }
        String concat2 = concat.concat("&").concat("port=").concat(DEFAULT_PROFILER_PORT);
        if (file.exists()) {
            try {
