...
public final class DebugStringsKt {
    ...
    public static final String toDebugString(Continuation<?> continuation) {
        String m413constructorimpl;
        if (continuation instanceof DispatchedContinuation) {
            return continuation.toString();
        }
        try {
            Result.Companion companion = Result.Companion;
            m413constructorimpl = Result.m413constructorimpl(continuation + '@' + getHexAddress(continuation));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            m413constructorimpl = Result.m413constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m416exceptionOrNullimpl(m413constructorimpl) != null) {
            m413constructorimpl = continuation.getClass().getName() + '@' + getHexAddress(continuation);
        }
        return (String) m413constructorimpl;
    }
