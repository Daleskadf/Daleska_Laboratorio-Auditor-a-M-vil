...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m2054constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m2054constructorimpl(CLOSED);
            }
    ...
    public static final boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, Segment segment) {
        while (true) {
            ...
            if (AbstractFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment2, segment)) {
    ...
    public static final Object findSegmentAndMoveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, long j, Segment segment, Function2 function2) {
        ...
        do {
            ...
            while (true) {
                Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                z = true;
                if (segment2.id < m2057getSegmentimpl.id) {
                    if (!m2057getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        z = false;
                        continue;
                        break;
                    } else if (AbstractFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment2, m2057getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                            continue;
                        } else {
            ...
            while (true) {
                ...
                if (segment2.id < m2057getSegmentimpl.id) {
                    if (!m2057getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        ...
                        continue;
                        break;
                    } else if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment2, m2057getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                            continue;
                        } else {
