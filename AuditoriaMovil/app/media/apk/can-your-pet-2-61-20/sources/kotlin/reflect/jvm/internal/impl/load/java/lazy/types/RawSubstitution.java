...
public final class RawSubstitution extends TypeSubstitution {
    ...
    @Override // kotlin.reflect.jvm.internal.impl.types.TypeSubstitution
    /* renamed from: get */
    public TypeProjectionImpl mo1751get(KotlinType key) {
    ...
    private final KotlinType eraseType(KotlinType kotlinType, JavaTypeAttributes javaTypeAttributes) {
        ...
        if (mo1746getDeclarationDescriptor instanceof TypeParameterDescriptor) {
            return eraseType(this.typeParameterUpperBoundEraser.getErasedUpperBound((TypeParameterDescriptor) mo1746getDeclarationDescriptor, javaTypeAttributes.markIsRaw(true)), javaTypeAttributes);
        }
        if (mo1746getDeclarationDescriptor instanceof ClassDescriptor) {
            ClassifierDescriptor mo1746getDeclarationDescriptor2 = FlexibleTypesKt.upperIfFlexible(kotlinType).getConstructor().mo1746getDeclarationDescriptor();
            if (!(mo1746getDeclarationDescriptor2 instanceof ClassDescriptor)) {
                throw new IllegalStateException(("For some reason declaration for upper bound is not a class but \"" + mo1746getDeclarationDescriptor2 + "\" while for lower it's \"" + mo1746getDeclarationDescriptor + Typography.quote).toString());
            }
            Pair<SimpleType, Boolean> eraseInflexibleBasedOnClassDescriptor = eraseInflexibleBasedOnClassDescriptor(FlexibleTypesKt.lowerIfFlexible(kotlinType), (ClassDescriptor) mo1746getDeclarationDescriptor, lowerTypeAttr);
            ...
            Pair<SimpleType, Boolean> eraseInflexibleBasedOnClassDescriptor2 = eraseInflexibleBasedOnClassDescriptor(FlexibleTypesKt.upperIfFlexible(kotlinType), (ClassDescriptor) mo1746getDeclarationDescriptor2, upperTypeAttr);
        ...
        throw new IllegalStateException(("Unexpected declaration kind: " + mo1746getDeclarationDescriptor).toString());
    }
    ...
    public final Pair<SimpleType, Boolean> eraseInflexibleBasedOnClassDescriptor(final SimpleType simpleType, final ClassDescriptor classDescriptor, final JavaTypeAttributes javaTypeAttributes) {
        ...
        if (KotlinBuiltIns.isArray(simpleType2)) {
            ...
            for (TypeParameterDescriptor parameter : list) {
                ...
                arrayList.add(ErasureProjectionComputer.computeProjection$default(this.projectionComputer, parameter, javaTypeAttributes, this.typeParameterUpperBoundEraser, null, 8, null));
            }
