                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (none == null) {
                        ...
                        for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                            SimpleTypeMarker mo1750transformType = none.mo1750transformType(typeCheckerState, kotlinTypeMarker);
                            if ((typeSystemContext.isClassType(mo1750transformType) && !typeSystemContext.isMarkedNullable(mo1750transformType)) || typeSystemContext.isDefinitelyNotNullType(mo1750transformType)) {
                                typeCheckerState.clear();
                            } else {
                                supertypesDeque.add(mo1750transformType);
                            }
                        }
                        continue;
                    }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                        SimpleTypeMarker mo1750transformType = supertypesPolicy.mo1750transformType(state, kotlinTypeMarker);
                        if (INSTANCE.isApplicableAsEndNode(state, mo1750transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo1750transformType);
