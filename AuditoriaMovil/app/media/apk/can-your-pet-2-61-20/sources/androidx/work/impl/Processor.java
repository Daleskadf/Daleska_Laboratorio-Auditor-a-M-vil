...
public class Processor implements ExecutionListener, ForegroundProcessor {
    ...
    public boolean startWork(StartStopToken startStopToken, WorkerParameters.RuntimeExtras runtimeExtras) {
        ...
        WorkSpec workSpec = (WorkSpec) this.mWorkDatabase.runInTransaction(new Callable() { // from class: androidx.work.impl.Processor$$ExternalSyntheticLambda0
            ...
            public final Object call() {
                return Processor.this.m185lambda$startWork$0$androidxworkimplProcessor(arrayList, workSpecId);
            }
        });
        if (workSpec == null) {
    ...
    public /* synthetic */ WorkSpec m185lambda$startWork$0$androidxworkimplProcessor(ArrayList arrayList, String str) throws Exception {
    ...
    public void startForeground(String workSpecId, ForegroundInfo foregroundInfo) {
        synchronized (this.mLock) {
            Logger logger = Logger.get();
            String str = TAG;
            logger.info(str, "Moving WorkSpec (" + workSpecId + ") to the foreground");
    ...
    public boolean stopAndCancelWork(String id) {
        ...
        synchronized (this.mLock) {
            Logger logger = Logger.get();
            String str = TAG;
            logger.debug(str, "Processor cancelling " + id);
    ...
    public void m184lambda$runOnExecuted$1$androidxworkimplProcessor(final WorkGenerationalId id, boolean needsReschedule) {
        synchronized (this.mLock) {
            ...
            Logger logger = Logger.get();
            String str = TAG;
            logger.debug(str, getClass().getSimpleName() + " " + id.getWorkSpecId() + " executed; reschedule = " + needsReschedule);
    ...
    private void runOnExecuted(final WorkGenerationalId id, final boolean needsReschedule) {
        this.mWorkTaskExecutor.getMainThreadExecutor().execute(new Runnable() { // from class: androidx.work.impl.Processor$$ExternalSyntheticLambda1
            ...
            public final void run() {
                Processor.this.m184lambda$runOnExecuted$1$androidxworkimplProcessor(id, needsReschedule);
            }
        });
    }
    ...
    private static boolean interrupt(String id, WorkerWrapper wrapper) {
        if (wrapper != null) {
            ...
            Logger logger = Logger.get();
            String str = TAG;
            logger.debug(str, "WorkerWrapper interrupted for " + id);
            return true;
        }
        Logger logger2 = Logger.get();
        String str2 = TAG;
        logger2.debug(str2, "WorkerWrapper could not be found for " + id);
        return false;
    }
