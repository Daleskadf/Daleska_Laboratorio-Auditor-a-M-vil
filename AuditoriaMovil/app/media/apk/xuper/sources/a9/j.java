...
public class j {
    ...
    public void b(a aVar, int i10, Buffer buffer, int i11, boolean z10) {
        if (a()) {
            Logger logger = this.f432a;
            Level level = this.f433b;
            logger.log(level, aVar + " DATA: streamId=" + i10 + " endStream=" + z10 + " length=" + i11 + " bytes=" + m(buffer));
        }
    }
    ...
    public void c(a aVar, int i10, c9.a aVar2, ByteString byteString) {
        if (a()) {
            Logger logger = this.f432a;
            Level level = this.f433b;
            logger.log(level, aVar + " GO_AWAY: lastStreamId=" + i10 + " errorCode=" + aVar2 + " length=" + byteString.size() + " bytes=" + m(new Buffer().write(byteString)));
        }
    }
