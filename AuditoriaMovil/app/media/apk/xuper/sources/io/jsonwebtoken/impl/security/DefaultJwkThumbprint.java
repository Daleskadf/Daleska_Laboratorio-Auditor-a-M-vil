...
class DefaultJwkThumbprint implements JwkThumbprint {
    ...
    public DefaultJwkThumbprint(byte[] bArr, HashAlgorithm hashAlgorithm) {
        ...
        this.uri = URI.create(URI_PREFIX + ((String) Assert.hasText(Strings.clean(hashAlgorithm.getId()), "Thumbprint HashAlgorithm id cannot be null or empty.")) + SOAP.DELIM + encoder.encode(bArr));
