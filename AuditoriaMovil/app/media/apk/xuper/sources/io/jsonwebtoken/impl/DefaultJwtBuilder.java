...
public class DefaultJwtBuilder implements JwtBuilder {
    ...
    public String compact() {
        ...
        if (z10) {
            return encrypt(payload, key, provider);
        }
        if (key != null) {
            return sign(payload, key, provider);
        }
    ...
    public JwtBuilder encodePayload(boolean z10) {
        ...
        LinkedHashSet linkedHashSet = new LinkedHashSet(Collections.nullSafe((Set) this.headerBuilder.get((Parameter<Object>) parameter)));
    ...
    private String encrypt(Payload payload, Key key, Provider provider) {
        ...
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream(8192);
        encodeAndWrite("JWE Protected Header", (JweHeader) Assert.isInstanceOf(JweHeader.class, this.headerBuilder.build(), "Invalid header created: "), byteArrayOutputStream);
        InputStream of = Streams.of(byteArrayOutputStream.toByteArray());
        ...
        byteArrayOutputStream.write(46);
        encodeAndWrite("JWE Encrypted CEK", (byte[]) Assert.notNull(apply.getPayload(), "KeyResult must return an encrypted key byte array, even if empty."), byteArrayOutputStream);
        byteArrayOutputStream.write(46);
