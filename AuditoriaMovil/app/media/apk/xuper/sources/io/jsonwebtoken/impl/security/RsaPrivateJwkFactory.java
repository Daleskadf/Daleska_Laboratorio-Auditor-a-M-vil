...
public class RsaPrivateJwkFactory extends AbstractFamilyJwkFactory<RSAPrivateKey, RsaPrivateJwk> {
    ...
    public RsaPrivateJwk createJwkFromKey(JwkContext<RSAPrivateKey> jwkContext) {
        RSAPublicKey derivePublic;
        RSAPrivateKey key = jwkContext.getKey();
        PublicKey publicKey = jwkContext.getPublicKey();
        if (publicKey != null) {
            derivePublic = (RSAPublicKey) Assert.isInstanceOf(RSAPublicKey.class, publicKey, PUBKEY_ERR_MSG);
        } else {
    ...
    public RsaPrivateJwk createJwkFromValues(JwkContext<RSAPrivateKey> jwkContext) {
        ...
        while (true) {
            if (!it.hasNext()) {
                z10 = false;
                break;
            } else if (jwkContext.containsKey(it.next().getId())) {
                z10 = true;
                break;
            }
        }
