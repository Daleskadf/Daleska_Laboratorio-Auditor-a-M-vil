...
public class LelinkRtspClient extends RtspClient {
    ...
    public LelinkRtspClient(ParamsMap paramsMap) {
        ...
        if (TextUtils.isEmpty(paramsMap.getStringParam(ParamsMap.DeviceParams.KEY_RAOP_PORT))) {
            ...
            stringParam = paramsMap.getStringParam(ParamsMap.DeviceParams.KEY_RAOP_PORT);
        }
        this.mRepoPort = Integer.parseInt(stringParam);
    ...
    public int sendRequestGetMirrorInfo() {
        ...
        if (this.versoinName.contains("5.3.2.9")) {
            protocal = new ProtocolBuilder().getMirrorInfoCmd().setLelinkDeviceId(this.mDeviceId).setLelinkDevNameCmd(this.mDeviceName).setLelinkprotocolVersion("0").setLelinkClientName(this.mSinkName).setStreamTime(str).setUserAgent(HAPPY_CAST).setContentLength("0").setAutorization(this.mScreenCode).setHapplay(HAPPY_VALUE).getProtocal(true);
        } else {
            protocal = new ProtocolBuilder().getMirrorInfoCmd().setAplleDevid(this.mDeviceId).setAppleDevName(this.mDeviceName).setApplePrcVs("0").setAppleClientName(this.mSinkName).setStreamTime(str).setUserAgent(HAPPY_CAST).setContentLength("0").setAutorization(this.mScreenCode).setHapplay(HAPPY_VALUE).getProtocal(true);
        }
        byte[] interactiveData = this.mVideoDataSender.interactiveData(protocal);
        if (interactiveData == null) {
            return 0;
        }
    ...
    public boolean sendRequestGetMirrorPort() {
        ...
        if (this.mMirrorPort == 0) {
            ...
            byte[] interactiveData2 = this.mVideoDataSender.interactiveData(protocal);
    ...
    public boolean sendRequestRecord() {
        ...
        this.cseq++;
        if (this.mProtocolSender.interactiveData(new ProtocolBuilder().getRecordCmd(this.mLocalip).setRange(ProtocolBuilder.VALUE_NTP).setRtpinfo(ProtocolBuilder.VALUE_RTPINFO).setRtspSeq(this.cseq + "").setLelinkDeviceId(this.mDeviceId).setRtspDacp(ProtocolBuilder.VALUE_DACP).setRtsActiveRemote(ProtocolBuilder.VALUE_ACTIVE_REMOTE).setUserAgent("AirPlay/150.33").getProtocal(true)) == null) {
            this.mProtocolSender.release();
            return false;
        }
