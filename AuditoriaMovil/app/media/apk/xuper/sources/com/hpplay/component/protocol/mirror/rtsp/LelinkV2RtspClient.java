...
public class LelinkV2RtspClient extends RtspClient {
    ...
    public int sendRequestGetMirrorInfo() {
        try {
            ...
            byte[] decryptData = this.mLelinkEncrypt.decryptData(this.mProtocolSender.interactiveEncryptData(this.mLelinkEncrypt.buildEncryptData(newClientCU.setNewCSEQ(this.cseq + "").setContentLength("0").getProtocal(true))));
    ...
    public boolean sendRequestRecord() {
        ...
        this.cseq++;
        if (this.mProtocolSender.interactiveEncryptData(this.mLelinkEncrypt.buildEncryptData(buildHeader(new NLProtocolBuiler().getRecordCmd(this.mLocalip, System.currentTimeMillis() + ""), 0).getBytes())) == null) {
            this.mProtocolSender.release();
            return false;
        }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:26:0x009e, code lost:
        r0 = e;
     */
    /* JADX WARN: Removed duplicated region for block: B:19:0x0070 A[Catch: Exception -> 0x009e, all -> 0x00a9, TryCatch #0 {Exception -> 0x009e, blocks: (B:17:0x006c, B:19:0x0070, B:21:0x0090), top: B:42:0x006c }] */
    @Override // com.hpplay.component.protocol.mirror.rtsp.RtspClient
    /*
    ...
    public int sendRequestVideoSetup(int i10, int i11) {
        ...
        if (decryptData != null && decryptData.length > 0) {
            ...
            if (!TextUtils.isEmpty(header)) {
                String str3 = header.split("\r\n")[0];
                CLog.i(TAG, "the header first line ----->" + str3);
                if (str3.contains(ProtocolBuilder.LELINK_UNSUPPORT_PREEMPT)) {
                    return 12;
                }
                if (str3.contains(ProtocolBuilder.LELINK_UNSUPPORT_FORBIDDEN)) {
                    return 13;
                }
            }
            NSDictionary nSDictionary = (NSDictionary) PropertyListParser.parse(ProtocolUtils.removeHeader(decryptData));
    ...
    public boolean setMirrorMode(String str) {
        ...
        try {
            if (this.mProtocolSender != null && !TextUtils.isEmpty(str)) {
                ...
                byte[] interactiveEncryptData = this.mProtocolSender.interactiveEncryptData(this.mLelinkEncrypt.buildEncryptData(mirrorMode.setNewCSEQ(this.cseq + "").setContentLength("0").getProtocal(true)));
                StringBuilder sb = new StringBuilder();
