    ...
    */
    ...
    private Object deserializeObject(NSDictionary nSDictionary, Class<?> cls, Type[] typeArr) {
        ...
        for (Map.Entry<String, NSObject> entry : hashMap.entrySet()) {
            ...
            if (method2 != null && method3 != null) {
                ...
                try {
                    method2.invoke(nSObject, toJavaObject(entry.getValue(), returnType, typeArr2));
    ...
    private Set<Object> deserializeSet() {
        ...
        if (set instanceof LinkedHashSet) {
            treeSet = new LinkedHashSet<>(set.size());
        } else {
            treeSet = new TreeSet<>();
        }
        for (NSObject nSObject : set) {
            treeSet.add(nSObject.toJavaObject());
        }
        return treeSet;
    }
    ...
    private Object deserializeArray(NSObject nSObject, Class<?> cls) {
        ...
        if (nSObject instanceof NSArray) {
            ...
            for (NSObject nSObject2 : set) {
                Array.set(newInstance2, i10, toJavaObject(nSObject2, classForName, null));
                i10++;
            }
            return newInstance2;
    ...
    /* JADX WARN: Removed duplicated region for block: B:22:0x0059 A[LOOP:0: B:20:0x0053->B:22:0x0059, LOOP_END] */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
