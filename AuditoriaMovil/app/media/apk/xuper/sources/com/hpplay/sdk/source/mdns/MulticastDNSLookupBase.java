                ...
                if (type != 12) {
                    if (type != 16) {
                        if (type != 28) {
                            if (type == 33) {
                                try {
                                    ServiceInstance serviceInstance = new ServiceInstance((SRVRecord) record);
                            ...
                            for (ServiceInstance serviceInstance2 : hashMap.values()) {
                                if (aAAARecord.getName().equals(serviceInstance2.getHost())) {
                                    if (aAAARecord.getTTL() > 0) {
                                        serviceInstance2.addAddress(aAAARecord.getAddress());
                                    } else {
                                        serviceInstance2.removeAddress(aAAARecord.getAddress());
                                    }
                                }
                            }
                        }
                    ...
                    ServiceInstance serviceInstance4 = (ServiceInstance) hashMap.get(pTRRecord.getTarget());
                    if (serviceInstance4 != null) {
                        if (pTRRecord.getTTL() > 0) {
                            serviceInstance4.addPointer(pTRRecord.getName());
                        } else {
                            serviceInstance4.removePointer(pTRRecord.getName());
                        }
                    }
                }
            } else {
                ARecord aRecord = (ARecord) record;
                for (ServiceInstance serviceInstance5 : hashMap.values()) {
                    if (aRecord.getName().equals(serviceInstance5.getHost())) {
                        if (aRecord.getTTL() > 0) {
                            serviceInstance5.addAddress(aRecord.getAddress());
                        } else {
                            serviceInstance5.removeAddress(aRecord.getAddress());
                        }
                    }
                }
            }
