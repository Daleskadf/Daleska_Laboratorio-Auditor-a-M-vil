...
public class DbOperations {
    ...
    private void fixDatabaseCode14() {
        ...
        try {
            OpenHelper helper = FlowManager.getDatabase(BBDatabase.class).getHelper();
            ...
            } else if (!TextUtils.equals(str, "Htv-6H")) {
                this.mFixDatabaseCode14 = true;
            } else {
                if (helper instanceof SQLiteOpenHelper) {
                    ((SQLiteOpenHelper) helper).setWriteAheadLoggingEnabled(false);
    ...
    public void deleteLimitNum() {
        synchronized (this) {
            ...
            FlowManager.getDatabase(BBDatabase.class).beginTransactionAsync(new ITransaction() { // from class: com.bigbee.db.DbOperations.3
    ...
    public void deleteListEvent(List<EventDbModel> list) {
        if (list != null && list.size() > 0) {
            ...
            synchronized (this) {
                try {
                    fixDatabaseCode14();
                    FlowManager.getDatabase(BBDatabase.class).executeTransaction(new ProcessModelTransaction.Builder(new ProcessModelTransaction.ProcessModel<EventDbModel>() { // from class: com.bigbee.db.DbOperations.5
    ...
    public void insertAll(List<EventDbModel> list) {
        if (list != null && list.size() > 0) {
            synchronized (this) {
                try {
                    fixDatabaseCode14();
                    FlowManager.getDatabase(BBDatabase.class).executeTransaction(new ProcessModelTransaction.Builder(new ProcessModelTransaction.ProcessModel<EventDbModel>() { // from class: com.bigbee.db.DbOperations.4
                        @Override // com.raizlabs.android.dbflow.structure.database.transaction.ProcessModelTransaction.ProcessModel
                        public void processModel(EventDbModel eventDbModel, DatabaseWrapper databaseWrapper) {
        ...
        synchronized (this) {
            try {
                try {
                    ...
                    From from = SQLite.select(new IProperty[0]).from(EventDbModel.class);
                    if (z10) {
                        where = from.where(EventDbModel_Table.notIntactEvent.notEq((Property<Boolean>) Boolean.TRUE));
                    } else {
                        where = from.where(EventDbModel_Table.notIntactEvent.notEq((Property<Boolean>) Boolean.TRUE), EventDbModel_Table.reporting.eq((Property<Boolean>) Boolean.FALSE));
                    }
                    queryList = where.limit(1000).orderBy(NameAlias.of(AnalyticsConfig.RTD_START_TIME), true).queryList();
                } catch (Exception e10) {
                    e10.printStackTrace();
                    return new ArrayList();
                }
    ...
    public void updateAllEvent(List<EventDbModel> list) {
        if (list != null && !list.isEmpty()) {
            synchronized (this) {
                try {
                    fixDatabaseCode14();
                    FlowManager.getDatabase(BBDatabase.class).executeTransaction(new ProcessModelTransaction.Builder(new ProcessModelTransaction.ProcessModel<EventDbModel>() { // from class: com.bigbee.db.DbOperations.7
                        @Override // com.raizlabs.android.dbflow.structure.database.transaction.ProcessModelTransaction.ProcessModel
                        public void processModel(EventDbModel eventDbModel, DatabaseWrapper databaseWrapper) {
    ...
    public void updateReportingStatus(List<EventDbModel> list, final boolean z10) {
        if (list != null && !list.isEmpty()) {
            synchronized (this) {
                try {
                    fixDatabaseCode14();
                    FlowManager.getDatabase(BBDatabase.class).executeTransaction(new ProcessModelTransaction.Builder(new ProcessModelTransaction.ProcessModel<EventDbModel>() { // from class: com.bigbee.db.DbOperations.6
                        @Override // com.raizlabs.android.dbflow.structure.database.transaction.ProcessModelTransaction.ProcessModel
                        public void processModel(EventDbModel eventDbModel, DatabaseWrapper databaseWrapper) {
                            if (eventDbModel != null) {
