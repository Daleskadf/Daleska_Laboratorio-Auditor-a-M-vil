...
public class FlowManager {
    ...
    public static synchronized void close() {
        synchronized (FlowManager.class) {
            for (Map.Entry<Class<?>, DatabaseDefinition> entry : globalDatabaseHolder.databaseClassLookupMap.entrySet()) {
    ...
    public static void init(FlowConfig flowConfig) {
        ...
        if (!flowConfig.databaseHolders().isEmpty()) {
            for (Class<? extends DatabaseHolder> cls : flowConfig.databaseHolders()) {
                loadDatabaseHolder(cls);
            }
        }
