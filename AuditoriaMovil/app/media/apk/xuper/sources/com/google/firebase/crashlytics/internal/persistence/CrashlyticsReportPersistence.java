...
public class CrashlyticsReportPersistence {
    ...
    private static final Comparator<? super File> LATEST_SESSION_ID_FIRST_COMPARATOR = new Comparator() { // from class: m4.c
    ...
    private static final FilenameFilter EVENT_FILE_FILTER = new FilenameFilter() { // from class: m4.d
    ...
    private void capFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles.subList(i10, size)) {
            file.delete();
        }
    }
    ...
    private void synthesizeReport(String str, long j10) {
        ...
        loop0: while (true) {
            ...
            for (File file : sessionFiles) {
                try {
                    ...
                    logger2.w("Could not add event to report for " + file, e10);
                }
        ...
        synthesizeReportFile(this.fileStore.getSessionFile(str, "report"), arrayList, j10, z10, UserMetadata.readUserId(str, this.fileStore));
    }

    private void synthesizeReportFile(File file, List<CrashlyticsReport.Session.Event> list, long j10, boolean z10, String str) {
        File report;
        try {
            CrashlyticsReportJsonTransform crashlyticsReportJsonTransform = TRANSFORM;
            ...
            Logger logger = Logger.getLogger();
            logger.w("Could not synthesize final report file for " + file, e10);
        }
    }

    private int trimEvents(String str, int i10) {
        List<File> sessionFiles = this.fileStore.getSessionFiles(str, new FilenameFilter() { // from class: m4.a
            @Override // java.io.FilenameFilter
        ...
        Collections.sort(sessionFiles, new Comparator() { // from class: m4.b
            @Override // java.util.Comparator
            public final int compare(Object obj, Object obj2) {
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File file : allFinalizedReportFiles) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.w("Could not load report file " + file + "; deleting", e10);
                file.delete();
            }
        }
        return arrayList;
    ...
    private static void writeTextFile(File file, String str, long j10) {
        OutputStreamWriter outputStreamWriter = new OutputStreamWriter(new FileOutputStream(file), UTF_8);
        try {
            outputStreamWriter.write(str);
