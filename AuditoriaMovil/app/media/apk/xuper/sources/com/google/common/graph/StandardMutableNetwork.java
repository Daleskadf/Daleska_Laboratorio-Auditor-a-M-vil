...
public final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n10, N n11, E e10) {
        ...
        if (!allowsParallelEdges()) {
            Preconditions.checkArgument((networkConnections == null || !networkConnections.successors().contains(n11)) ? true : true, "Nodes %s and %s are already connected by a different edge. To construct a graph that allows parallel edges, call allowsParallelEdges(true) on the Builder.", n10, n11);
        }
