...
public final class BloomFilter<T> implements Predicate<T>, Serializable {
    ...
    public long approximateElementCount() {
        ...
        Double.isNaN(d10);
        Double.isNaN(d10);
        double d11 = (-Math.log1p(-(bitCount / d10))) * d10;
        double d12 = this.numHashFunctions;
        Double.isNaN(d12);
        return DoubleMath.roundToLong(d11 / d12, RoundingMode.HALF_UP);
    }
    ...
    public void putAll(BloomFilter<T> bloomFilter) {
        ...
        Preconditions.checkArgument(z11, "BloomFilters must have the same number of hash functions (%s != %s)", i10, i11);
        if (bitSize() == bloomFilter.bitSize()) {
            z12 = true;
        } else {
            z12 = false;
        }
        Preconditions.checkArgument(z12, "BloomFilters must have the same size underlying bit arrays (%s != %s)", bitSize(), bloomFilter.bitSize());
