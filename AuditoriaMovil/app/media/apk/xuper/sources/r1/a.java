            ...
            try {
                if (file.exists() && !file.isDirectory()) {
                    if (s.e(str, ".gif", false, 2, null)) {
                        ...
                        if (num2 != null) {
                            RequestBuilder override = requestBuilder.override(num2.intValue(), num2.intValue());
                            i.f(override, "requestBuilder.override(size,size)");
                            requestBuilder = override;
                        }
                        if (z10) {
                            RequestBuilder dontAnimate = requestBuilder.dontAnimate();
                            i.f(dontAnimate, "requestBuilder.dontAnimate()");
                            requestBuilder = dontAnimate;
                        }
                        if (num != null) {
                            Cloneable error = requestBuilder.placeholder(num.intValue()).error(num.intValue());
                    ...
                    if (num2 != null) {
                        Cloneable override2 = load.override(num2.intValue(), num2.intValue());
                        i.f(override2, "requestBuilder.override(size,size)");
                        load = (RequestBuilder) override2;
                    }
                    if (z10) {
                        Cloneable dontAnimate2 = load.dontAnimate();
                        i.f(dontAnimate2, "requestBuilder.dontAnimate()");
                        load = (RequestBuilder) dontAnimate2;
                    }
                    if (num != null) {
                        Cloneable error2 = load.placeholder(num.intValue()).error(num.intValue());
