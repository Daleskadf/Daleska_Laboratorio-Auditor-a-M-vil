...
public final class zzmt {
    ...
    private static zzms zza(zzms zzmsVar, zzms zzmsVar2, BigInteger bigInteger, BigInteger bigInteger2) {
        if (zzmsVar.zza()) {
            return zzmsVar2;
        }
        if (zzmsVar2.zza()) {
        ...
        BigInteger bigInteger3 = zzmsVar.zzd;
        BigInteger mod = bigInteger3.multiply(bigInteger3).mod(bigInteger2);
        BigInteger bigInteger4 = zzmsVar2.zzd;
        BigInteger mod2 = bigInteger4.multiply(bigInteger4).mod(bigInteger2);
        BigInteger mod3 = zzmsVar.zzb.multiply(mod2).mod(bigInteger2);
        BigInteger mod4 = zzmsVar2.zzb.multiply(mod).mod(bigInteger2);
        BigInteger mod5 = zzmsVar.zzc.multiply(zzmsVar2.zzd).mod(bigInteger2).multiply(mod2).mod(bigInteger2);
        BigInteger mod6 = zzmsVar2.zzc.multiply(zzmsVar.zzd).mod(bigInteger2).multiply(mod).mod(bigInteger2);
        if (mod3.equals(mod4)) {
            if (!mod5.equals(mod6)) {
                return zzms.zza;
            }
            return zza(zzmsVar, bigInteger, bigInteger2);
        }
        ...
        BigInteger add = zzmsVar.zzd.add(zzmsVar2.zzd);
        return new zzms(mod12, mod13, add.multiply(add).mod(bigInteger2).subtract(mod).subtract(mod2).multiply(mod7).mod(bigInteger2));
    }

    private static zzms zza(zzms zzmsVar, BigInteger bigInteger, BigInteger bigInteger2) {
        if (zzmsVar.zzc.equals(BigInteger.ZERO)) {
            return zzms.zza;
        }
        BigInteger bigInteger3 = zzmsVar.zzb;
        BigInteger mod = bigInteger3.multiply(bigInteger3).mod(bigInteger2);
        BigInteger bigInteger4 = zzmsVar.zzc;
        BigInteger mod2 = bigInteger4.multiply(bigInteger4).mod(bigInteger2);
        BigInteger mod3 = mod2.multiply(mod2).mod(bigInteger2);
        BigInteger bigInteger5 = zzmsVar.zzd;
        BigInteger mod4 = bigInteger5.multiply(bigInteger5).mod(bigInteger2);
        BigInteger add = zzmsVar.zzb.add(mod2);
        ...
        BigInteger add3 = zzmsVar.zzc.add(zzmsVar.zzd);
        return new zzms(mod5, mod6, add3.multiply(add3).mod(bigInteger2).subtract(mod2).subtract(mod4).mod(bigInteger2));
    }

    private static zzms zza(ECPoint eCPoint, BigInteger bigInteger) {
        if (eCPoint.equals(ECPoint.POINT_INFINITY)) {
            return zzms.zza;
        }
        BigInteger mod = new BigInteger(1, zzpp.zza((bigInteger.bitLength() + 8) / 8)).mod(bigInteger);
        BigInteger mod2 = mod.multiply(mod).mod(bigInteger);
        return new zzms(eCPoint.getAffineX().multiply(mod2).mod(bigInteger), eCPoint.getAffineY().multiply(mod2.multiply(mod).mod(bigInteger)).mod(bigInteger), mod);
    }
    ...
    public static ECPoint zza(BigInteger bigInteger, ECParameterSpec eCParameterSpec) {
        ...
        if (bigInteger.signum() == 1) {
            if (bigInteger.compareTo(eCParameterSpec.getOrder()) < 0) {
                ...
                zzms zza3 = zza(ECPoint.POINT_INFINITY, zza2);
                zzms zza4 = zza(generator, zza2);
                for (int bitLength = bigInteger.bitLength(); bitLength >= 0; bitLength--) {
                    if (bigInteger.testBit(bitLength)) {
                        zza3 = zza(zza3, zza4, a7, zza2);
                        zza4 = zza(zza4, a7, zza2);
                    } else {
                        zza4 = zza(zza3, zza4, a7, zza2);
                        zza3 = zza(zza3, a7, zza2);
                    }
                }
                if (zza3.zza()) {
