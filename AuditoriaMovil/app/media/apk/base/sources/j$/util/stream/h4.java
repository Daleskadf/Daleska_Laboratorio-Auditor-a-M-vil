package j$.util.stream;

import j$.util.AbstractC1210d;
import j$.util.Spliterator;
import java.util.function.Consumer;
import java.util.function.LongConsumer;
import java.util.function.LongPredicate;
/* loaded from: classes2.dex */
final class h4 extends j4 implements LongConsumer, j$.util.c0 {

    /* renamed from: e  reason: collision with root package name */
    long f13279e;
    public final /* synthetic */ int f;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ h4(Spliterator spliterator, int i7) {
        super(spliterator);
        this.f = i7;
    }

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ h4(Spliterator spliterator, j4 j4Var, int i7) {
        super(spliterator, j4Var);
        this.f = i7;
    }

    public final /* synthetic */ LongConsumer andThen(LongConsumer longConsumer) {
        return j$.com.android.tools.r8.a.e(this, longConsumer);
    }

    @Override // j$.util.stream.j4, j$.util.Spliterator
    public final /* synthetic */ void forEachRemaining(Consumer consumer) {
        AbstractC1210d.c(this, consumer);
    }

    @Override // j$.util.Spliterator
    public final /* synthetic */ boolean tryAdvance(Consumer consumer) {
        return AbstractC1210d.h(this, consumer);
    }

    @Override // j$.util.f0
    public final void forEachRemaining(LongConsumer longConsumer) {
        do {
        } while (tryAdvance(longConsumer));
    }

    @Override // java.util.function.LongConsumer
    public final void accept(long j) {
        this.f13292d = (this.f13292d + 1) & 63;
        this.f13279e = j;
    }

    @Override // j$.util.stream.j4
    final Spliterator c(Spliterator spliterator) {
        switch (this.f) {
            case 0:
                return new h4((j$.util.c0) spliterator, this, 0);
            default:
                return new h4((j$.util.c0) spliterator, this, 1);
        }
    }

    @Override // j$.util.stream.j4, j$.util.Spliterator
    public /* bridge */ /* synthetic */ Spliterator trySplit() {
        switch (this.f) {
            case 1:
                return trySplit();
            default:
                return super.trySplit();
        }
    }

    @Override // j$.util.stream.j4, j$.util.Spliterator
    public /* bridge */ /* synthetic */ j$.util.f0 trySplit() {
        switch (this.f) {
            case 1:
                return trySplit();
            default:
                return super.trySplit();
        }
    }

    @Override // j$.util.c0
    public final boolean tryAdvance(LongConsumer longConsumer) {
        switch (this.f) {
            case 0:
                boolean z7 = this.f13291c;
                Spliterator spliterator = this.f13289a;
                if (z7) {
                    this.f13291c = false;
                    boolean tryAdvance = ((j$.util.c0) spliterator).tryAdvance((LongConsumer) this);
                    if (tryAdvance && b()) {
                        LongPredicate longPredicate = null;
                        longPredicate.test(this.f13279e);
                        throw null;
                    } else if (tryAdvance) {
                        longConsumer.accept(this.f13279e);
                        return tryAdvance;
                    } else {
                        return tryAdvance;
                    }
                }
                return ((j$.util.c0) spliterator).tryAdvance(longConsumer);
            default:
                if (this.f13291c && b() && ((j$.util.c0) this.f13289a).tryAdvance((LongConsumer) this)) {
                    LongPredicate longPredicate2 = null;
                    longPredicate2.test(this.f13279e);
                    throw null;
                }
                this.f13291c = false;
                return false;
        }
    }

    @Override // j$.util.stream.j4, j$.util.Spliterator
    public j$.util.c0 trySplit() {
        switch (this.f) {
            case 1:
                if (this.f13290b.get()) {
                    return null;
                }
                return (j$.util.c0) super.trySplit();
            default:
                return super.trySplit();
        }
    }

    @Override // j$.util.f0
    public /* bridge */ /* synthetic */ boolean tryAdvance(Object obj) {
        switch (this.f) {
            case 1:
                tryAdvance((LongConsumer) obj);
                return false;
            default:
                return tryAdvance((LongConsumer) obj);
        }
    }
}
