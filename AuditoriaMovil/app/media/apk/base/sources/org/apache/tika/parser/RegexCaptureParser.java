...
public class RegexCaptureParser implements Parser, Initializable {
    ...
    public void parse(InputStream inputStream, ContentHandler contentHandler, Metadata metadata, ParseContext parseContext) {
        ...
        try {
            ...
            for (Map.Entry<String, Pattern> entry : this.captureMap.entrySet()) {
                hashMap.put(entry.getKey(), entry.getValue().matcher(StringUtils.EMPTY));
            }
            HashMap hashMap2 = new HashMap();
            for (Map.Entry<String, Pattern> entry2 : this.matchMap.entrySet()) {
                hashMap2.put(entry2.getKey(), entry2.getValue().matcher(StringUtils.EMPTY));
            }
    ...
    @Field
    public void setWriteContent(boolean z7) {
        this.writeContent = z7;
    }
}
