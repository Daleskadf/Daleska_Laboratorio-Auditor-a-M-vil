...
public class LinkContentHandler extends DefaultHandler {
    ...
    public void startElement(String str, String str2, String str3, Attributes attributes) {
        if (XHTMLContentHandler.XHTML.equals(str)) {
            if ("a".equals(str2)) {
                ...
                linkBuilder.setURI(attributes.getValue(StringUtils.EMPTY, "href"));
                linkBuilder.setTitle(attributes.getValue(StringUtils.EMPTY, "title"));
                linkBuilder.setRel(attributes.getValue(StringUtils.EMPTY, "rel"));
                this.builderStack.addFirst(linkBuilder);
            } else if ("link".equals(str2)) {
                LinkBuilder linkBuilder2 = new LinkBuilder("link");
                linkBuilder2.setURI(attributes.getValue(StringUtils.EMPTY, "href"));
                linkBuilder2.setRel(attributes.getValue(StringUtils.EMPTY, "rel"));
                this.builderStack.addFirst(linkBuilder2);
            } else if ("script".equals(str2)) {
                if (attributes.getValue(StringUtils.EMPTY, "src") != null) {
                    LinkBuilder linkBuilder3 = new LinkBuilder("script");
                    linkBuilder3.setURI(attributes.getValue(StringUtils.EMPTY, "src"));
                    this.builderStack.addFirst(linkBuilder3);
                }
            } else if ("iframe".equals(str2)) {
                LinkBuilder linkBuilder4 = new LinkBuilder("iframe");
                linkBuilder4.setURI(attributes.getValue(StringUtils.EMPTY, "src"));
                this.builderStack.addFirst(linkBuilder4);
            } else if ("img".equals(str2)) {
                LinkBuilder linkBuilder5 = new LinkBuilder("img");
                linkBuilder5.setURI(attributes.getValue(StringUtils.EMPTY, "src"));
                linkBuilder5.setTitle(attributes.getValue(StringUtils.EMPTY, "title"));
                linkBuilder5.setRel(attributes.getValue(StringUtils.EMPTY, "rel"));
                this.builderStack.addFirst(linkBuilder5);
                String value = attributes.getValue(StringUtils.EMPTY, "alt");
                if (value != null) {
    ...
    public LinkContentHandler(boolean z7) {
        this.builderStack = new LinkedList<>();
        this.links = new ArrayList();
        this.collapseWhitespaceInAnchor = z7;
    }
}
