...
public final class MimeTypes implements Detector, Serializable {
    ...
    */
    ...
    public MimeType forName(String str) {
        ...
        throw new MimeTypeException(b.h("Invalid media type name: ", str));
    }
    ...
    public byte[] readMagicHeader(InputStream inputStream) {
        if (inputStream != null) {
            ...
            while (read != -1) {
                ...
                read = inputStream.read(bArr, i7, minLength - i7);
            }
        ...
        synchronized (MimeTypes.class) {
            ...
            if (mimeTypes == null) {
                try {
                    ...
                    throw new RuntimeException("Unable to parse the default media type registry", e8);
                }
            }
        }
        return mimeTypes;
    ...
    public void addPattern(MimeType mimeType, String str, boolean z7) {
        this.patterns.add(str, z7, mimeType);
    }
    ...
    /* JADX WARN: Type inference failed for: r1v4, types: [W6.g, T6.d] */
    public List<MimeType> getMimeType(byte[] bArr) {
        if (bArr != null) {
            ...
            for (Magic magic : this.magics) {
                if (i7 > 0 && i7 > magic.getPriority()) {
                    break;
                } else if (magic.eval(bArr)) {
                    arrayList.add(magic.getType());
                    i7 = magic.getPriority();
                }
            }
            if (!arrayList.isEmpty()) {
                for (int i8 = 0; i8 < arrayList.size(); i8++) {
                    MimeType mimeType = (MimeType) arrayList.get(i8);
                            ...
                            while (true) {
                                if (it2.hasNext()) {
                                    ...
                                    if (next2.getType().equals(this.htmlMimeType) && next2.eval(bArr)) {
                                        arrayList.set(i8, this.htmlMimeType);
                                        break;
                                    }
                                } else {
                                    arrayList.set(i8, this.textMimeType);
                                    break;
                                }
                            }
                        }
