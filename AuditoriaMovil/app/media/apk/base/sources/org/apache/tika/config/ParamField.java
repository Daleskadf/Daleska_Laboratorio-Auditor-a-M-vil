...
public class ParamField {
    ...
    private String retrieveParamName(Field field) {
        if (field.name().equals(DEFAULT)) {
            ...
            if (name.startsWith("set") && name.length() > 3) {
                return AbstractC0059i.z(name.substring(3, 4).toLowerCase(Locale.ROOT), name.substring(4));
            }
        ...
        if (field != null) {
            ...
            if (parameterTypes.length != 1) {
                if (this.setter.getName().startsWith("get")) {
                    str = AbstractC0059i.M("Invalid setter method. Must have one and only one parameter. Perhaps the annotation is misplaced on ", this.setter.getName(), " while a set'X' is expected?");
                } else {
    ...
    public String toString() {
        ...
        return "ParamField{name='" + str + "', type=" + cls + ", required=" + z7 + "}";
    }
}
