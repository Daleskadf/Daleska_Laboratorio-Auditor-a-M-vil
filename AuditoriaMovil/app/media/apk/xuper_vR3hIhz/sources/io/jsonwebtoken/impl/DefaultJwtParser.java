...
public class DefaultJwtParser extends AbstractParser<Jwt<?, ?>> implements JwtParser {
    ...
    private void validateExpectedClaims(Header header, Claims claims) {
        ...
        for (String str2 : build.keySet()) {
            Object normalize = normalize(build.get(str2));
            ...
            if (normalize2 == null) {
                ...
                if (normalize2 instanceof Collection) {
                    of = (Collection) normalize2;
                } else {
                    of = Collections.setOf(normalize2);
                }
                for (Object obj : collection) {
                    if (!Collections.contains(of.iterator(), obj)) {
                        throw new IncorrectClaimException(header, claims, str2, normalize, String.format(MISSING_EXPECTED_CLAIM_VALUE_MESSAGE_TEMPLATE, obj, str2, of));
                    }
                }
                continue;
            } else if (!normalize.equals(normalize2)) {
                throw new IncorrectClaimException(header, claims, str2, normalize, String.format("Expected %s claim to be: %s, but was: %s.", str2, normalize, normalize2));
            }
        }
    }
    ...
    private byte[] verifySignature(TokenizedJwt tokenizedJwt, JwsHeader jwsHeader, String str, SigningKeyResolver signingKeyResolver, Claims claims, Payload payload) {
        ...
        try {
            ...
            if (claims != null) {
                resolveSigningKey = signingKeyResolver.resolveSigningKey(jwsHeader, claims);
            } else {
                resolveSigningKey = signingKeyResolver.resolveSigningKey(jwsHeader, payload.getBytes());
            }
    ...
    public byte[] decode(CharSequence charSequence, String str) {
        ...
        try {
            ...
            if ("payload".equals(str)) {
                charSequence2 = RedactedSupplier.REDACTED_VALUE;
            } else {
                charSequence2 = charSequence.toString();
            }
            throw new MalformedJwtException("Invalid Base64Url " + str + ": " + charSequence2, th);
        }
    }
    ...
    /* JADX WARN: Removed duplicated region for block: B:163:0x03c9 A[Catch: all -> 0x03d1, TRY_ENTER, TRY_LEAVE, TryCatch #2 {all -> 0x03f6, blocks: (B:154:0x03b7, B:169:0x03ef, B:160:0x03c2, B:163:0x03c9), top: B:232:0x0393 }] */
    /* JADX WARN: Removed duplicated region for block: B:169:0x03ef A[Catch: all -> 0x03f6, TRY_LEAVE, TryCatch #2 {all -> 0x03f6, blocks: (B:154:0x03b7, B:169:0x03ef, B:160:0x03c2, B:163:0x03c9), top: B:232:0x0393 }] */
    ...
    /* JADX WARN: Removed duplicated region for block: B:236:0x038b A[EXC_TOP_SPLITTER, SYNTHETIC] */
