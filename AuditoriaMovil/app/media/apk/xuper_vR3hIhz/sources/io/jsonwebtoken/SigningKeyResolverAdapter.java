...
public class SigningKeyResolverAdapter implements SigningKeyResolver {
    ...
    public Key resolveSigningKey(JwsHeader jwsHeader, byte[] bArr) {
        SignatureAlgorithm forName = SignatureAlgorithm.forName(jwsHeader.getAlgorithm());
        boolean isHmac = forName.isHmac();
        Assert.isTrue(isHmac, "The default resolveSigningKey(JwsHeader, byte[]) implementation cannot be used for asymmetric key algorithms (RSA, Elliptic Curve).  Override the resolveSigningKey(JwsHeader, byte[]) method instead and return a Key instance appropriate for the " + forName.name() + " algorithm.");
