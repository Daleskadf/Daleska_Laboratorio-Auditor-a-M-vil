...
public class DefaultDynamicJwkBuilder<K extends Key, J extends Jwk<K>> extends AbstractJwkBuilder<K, J, DynamicJwkBuilder<K, J>> implements DynamicJwkBuilder<K, J> {
    ...
    public <A extends PublicKey, B extends PrivateKey> PublicJwkBuilder<A, B, ?, ?, ?, ?> chain(List<X509Certificate> list) {
        ...
        return (PublicJwkBuilder) key((DefaultDynamicJwkBuilder<K, J>) ((PublicKey) Assert.notNull(((X509Certificate) Assert.notNull(list.get(0), "The first X509Certificate cannot be null.")).getPublicKey(), "The first X509Certificate's PublicKey cannot be null."))).x509Chain(list);
    }
    ...
    public <A extends PublicKey, B extends PrivateKey> PrivateJwkBuilder<B, A, ?, ?, ?> keyPair(KeyPair keyPair) {
        return (PrivateJwkBuilder<B, A, ?, ?, ?>) key((DefaultDynamicJwkBuilder<K, J>) ((PrivateKey) KeyPairs.getKey(keyPair, PrivateKey.class))).publicKey((A) KeyPairs.getKey(keyPair, PublicKey.class));
    }
    ...
    public <A extends PrivateKey, B extends PublicKey> OctetPrivateJwkBuilder<A, B> octetKeyPair(KeyPair keyPair) {
        B b10 = (B) KeyPairs.getKey(keyPair, PublicKey.class);
        PrivateKey privateKey = (PrivateKey) KeyPairs.getKey(keyPair, PrivateKey.class);
        EdwardsCurve.assertEdwards(b10);
        EdwardsCurve.assertEdwards(privateKey);
        return (OctetPrivateJwkBuilder) octetKey((DefaultDynamicJwkBuilder<K, J>) privateKey).publicKey(b10);
    }
    ...
    public J build() {
        if (Strings.hasText((String) ((JwkContext) this.DELEGATE).get((Parameter<Object>) AbstractJwk.KTY))) {
            T t10 = this.DELEGATE;
            setDelegate(this.jwkFactory.newContext((JwkContext) t10, ((JwkContext) t10).getKey()));
        }
