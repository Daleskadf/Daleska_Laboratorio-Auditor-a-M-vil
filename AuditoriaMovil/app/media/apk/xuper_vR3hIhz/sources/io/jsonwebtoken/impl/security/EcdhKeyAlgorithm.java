...
class EcdhKeyAlgorithm extends CryptoAlgorithm implements KeyAlgorithm<PublicKey, PrivateKey> {
    ...
    private int getKeyBitLength(AeadAlgorithm aeadAlgorithm) {
        ...
        if (keyAlgorithm instanceof KeyLengthSupplier) {
            keyBitLength = ((KeyLengthSupplier) keyAlgorithm).getKeyBitLength();
        } else {
            keyBitLength = aeadAlgorithm.getKeyBitLength();
        }
        return ((Integer) Assert.gt(Integer.valueOf(keyBitLength), 0, "Algorithm keyBitLength must be > 0")).intValue();
    }
    ...
    public KeyResult getEncryptionKey(KeyRequest<PublicKey> keyRequest) {
        ...
        ((JweHeader) Assert.notNull(keyRequest.getHeader(), "Request JweHeader cannot be null.")).put(DefaultJweHeader.EPK.getId(), (PublicJwk) ((DynamicJwkBuilder) Jwks.builder().random(ensureSecureRandom)).key((DynamicJwkBuilder) generateKeyPair.getPublic()).build());
