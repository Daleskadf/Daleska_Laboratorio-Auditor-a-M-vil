...
public class AesGcmKeyAlgorithm extends AesAlgorithm implements SecretKeyAlgorithm {
    ...
    public SecretKey getDecryptionKey(DecryptionKeyRequest<SecretKey> decryptionKeyRequest) {
        ...
        final byte[] concat = Bytes.concat(notEmpty, (byte[]) requiredParameterReader.get(DefaultJweHeader.TAG));
    ...
    public KeyResult getEncryptionKey(KeyRequest<SecretKey> keyRequest) {
        ...
        jweHeader.put(DefaultJweHeader.IV.getId(), encoder.encode(ensureInitializationVector));
        jweHeader.put(DefaultJweHeader.TAG.getId(), encoder.encode(bArr3));
