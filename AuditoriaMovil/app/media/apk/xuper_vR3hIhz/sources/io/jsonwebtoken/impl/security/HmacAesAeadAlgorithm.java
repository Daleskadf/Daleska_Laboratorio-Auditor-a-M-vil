...
public class HmacAesAeadAlgorithm extends AesAlgorithm implements AeadAlgorithm {
    ...
    public void decrypt(DecryptAeadRequest decryptAeadRequest, final OutputStream outputStream) {
        ...
        try {
            ...
            if (associatedData == null) {
                ...
                bytes = Streams.bytes(associatedData, "Unable to read AAD bytes.");
            }
            try {
                byte[] sign = sign(bytes, assertDecryptionIv, inputStream, copyOfRange);
    ...
    public void encrypt(AeadRequest aeadRequest, AeadResult aeadResult) {
        ...
        try {
            ...
            try {
                aeadResult.setTag(sign(bytes, ensureInitializationVector, Streams.of(byteArrayOutputStream.toByteArray()), copyOfRange)).setIv(ensureInitializationVector);
            } finally {
                Bytes.clear(copyOfRange);
            }
        } catch (Throwable th) {
