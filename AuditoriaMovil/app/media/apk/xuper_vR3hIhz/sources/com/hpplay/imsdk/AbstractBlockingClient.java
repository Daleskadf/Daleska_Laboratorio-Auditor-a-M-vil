...
public abstract class AbstractBlockingClient extends Observable implements Runnable {
    ...
    public synchronized Boolean authWrite() {
        ...
        this.out.get().write(BruteForceCoding.add(bArr, (this.uid + ";" + this.capability + ";" + this.appid + ";" + this.token).getBytes()));
        this.out.get().flush();
        return Boolean.TRUE;
    }
    ...
    /* JADX WARN: Removed duplicated region for block: B:82:0x01b4 A[Catch: Exception -> 0x01ba, TRY_LEAVE, TryCatch #3 {Exception -> 0x01ba, blocks: (B:76:0x0186, B:80:0x01a7, B:81:0x01ae, B:82:0x01b4), top: B:104:0x0186 }] */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:75:0x0183 -> B:104:0x0186). Please submit an issue!!! */
    @Override // java.lang.Runnable
    /*
