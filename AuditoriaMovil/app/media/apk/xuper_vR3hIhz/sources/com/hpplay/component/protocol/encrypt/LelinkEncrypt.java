...
public class LelinkEncrypt {
    ...
    public String buildHeader(NLProtocolBuiler nLProtocolBuiler, int i10) {
        NLProtocolBuiler contentType = nLProtocolBuiler.setPlatfrom().setUserAgent("HappyCast5,0/500.0").setNewLelinkClientId("0xff99ffex0022").setNewSessionId(this.mSessionId).setContentType("application/octet-stream");
        return contentType.setContentLength(i10 + "").getString(true);
    }

    public String bytesToHex(byte[] bArr) {
        if (this.isDebug) {
            return ProtocolUtils.bytesToHex(bArr);
        }
        return "";
    }
        ...
        if (body != null && !this.isRelase) {
            ...
            if (i10 != 0) {
                if (i10 == 1 || i10 == 2) {
                    ...
                    byte[] bytesValue = parse.getBytesValue(6);
                    ...
                    try {
                        this.srp6ClientSession.step2(SRP6CryptoParams.getInstance(2048, "SHA-1"), BigIntegerUtils.bigIntegerFromBytes(bytesValue), BigIntegerUtils.bigIntegerFromBytes(this.mRcvSrpPuk));
                    } catch (SRP6Exception e10) {
                        CLog.w(TAG, e10);
                    }
                }
            } else {
                byte[] bytesValue2 = parse.getBytesValue(3);
                byte[] bArr2 = this.mRcvEdPk;
                System.arraycopy(bytesValue2, 0, bArr2, 0, bArr2.length);
                byte[] bArr3 = this.mRcvEdPkSalt;
                System.arraycopy(bytesValue2, 32, bArr3, 0, bArr3.length);
            }
            return true;
        }
        this.mEncryptState = "failed";
            ...
            if (i10 != 0) {
                if (i10 == 1 || i10 == 2) {
                    try {
                        ...
                        byte[] bArr2 = new byte[64];
                        this.msrpSignMessage = bArr2;
                        System.arraycopy(this.mSrpPukey, 0, bArr2, 0, 32);
                        System.arraycopy(this.mRcvSrpPuk, 0, this.msrpSignMessage, 32, 32);
                        byte[] bArr3 = new byte[64];
                        this.mChachaSignMessage = bArr3;
                        System.arraycopy(this.msrpSignMessage, 0, bArr3, 0, bArr3.length);
                ...
                byte[] bArr4 = new byte[32];
                this.mRcvCurvePkey = bArr4;
                this.mRcvSignatrue = new byte[64];
                System.arraycopy(bytesValue, 0, bArr4, 0, bArr4.length);
                byte[] bArr5 = this.mRcvSignatrue;
                System.arraycopy(bytesValue, 32, bArr5, 0, bArr5.length);
                byte[] bArr6 = new byte[32];
                this.sharedSecret = bArr6;
                Curve25519.curve(bArr6, this.mCurvePrkey, this.mRcvCurvePkey);
                try {
                    byte[] bArr7 = new byte[64];
                    this.mEd25519Encrypt.aecrypt(genSha512(SALT_KEY.getBytes(), this.sharedSecret, 16), genSha512(SALT_IV.getBytes(), this.sharedSecret, 16), this.mRcvSignatrue, 0, 64, bArr7, 0, false);
                    byte[] bArr8 = new byte[64];
                    byte[] bArr9 = this.mRcvCurvePkey;
                    System.arraycopy(bArr9, 0, bArr8, 0, bArr9.length);
                    byte[] bArr10 = this.mCurvePukey;
                    System.arraycopy(bArr10, 0, bArr8, 32, bArr10.length);
                    if (this.mEd25519Encrypt.verify(this.mRcvEdPk, bArr8, bArr7)) {
            ...
            if (i10 != 0) {
                if (i10 == 1 || i10 == 2) {
                    ...
                    if (chachaDecrypt(bytesValue, bytesValue2)) {
                        ...
                        byte[] genSha512 = genSha512(this.mSrpSalt, bytesValue3, 32);
                        byte[] bArr2 = new byte[32];
                        System.arraycopy(this.srp6ClientSession.getSessionKeyHash(), 0, bArr2, 0, 32);
                        byte[] genSha5122 = genSha512(bArr2, genSha512(bArr2, genSha512, 32), 32);
                        this.chachaMaterKey = genSha512(genSha5122, SRP_IDE_SALT_KEY.getBytes(), 32);
                        this.chachaNonce = genSha512(genSha5122, SRP_IDE_NONCE.getBytes(), 32);
                    }
                }
            } else {
                this.mRcvType = parse.getByteValue(1).byteValue();
                ...
                byte[] genSha5123 = genSha512(this.sharedSecret, genSha512(this.sharedSecret, genSha512(this.mKeySalt, this.mRcvEdPkSalt, 32), 32), 32);
                this.chachaMaterKey = genSha512(genSha5123, IDE_SALT_KEY.getBytes(), 32);
                this.chachaNonce = genSha512(genSha5123, IDE_NONCE.getBytes(), 32);
            }
            return true;
        }
        this.mEncryptState = "failed";
