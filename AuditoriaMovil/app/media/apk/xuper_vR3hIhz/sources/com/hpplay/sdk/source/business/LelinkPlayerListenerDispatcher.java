...
public class LelinkPlayerListenerDispatcher {
    ...
    private DaCastBean createDaCastBean(OutParameter outParameter) {
        if (outParameter == null) {
            return new DaCastBean();
        }
        return new DaCastBean();
    }

    private boolean isSamePlayInfo(OutParameter outParameter, OutParameter outParameter2) {
        if (outParameter == null || outParameter2 == null || !TextUtils.equals(outParameter.urlID, outParameter2.urlID) || outParameter.castType != outParameter2.castType || outParameter.mimeType != outParameter2.mimeType) {
            return false;
        }
        return true;
    }
    ...
    public void onLoading(OutParameter outParameter) {
        ...
        } else if (outParameter != null && outParameter.pushType == 1) {
            IDaPlayerListener iDaPlayerListener = this.mOuterDaPlayerListener;
            if (iDaPlayerListener != null) {
    ...
    public void onStop(OutParameter outParameter) {
        ...
        } else if (!CastUtil.isSupportCloudMultiCast() && !isSamePlayInfo(outParameter, this.mCurrentPlayInfo)) {
