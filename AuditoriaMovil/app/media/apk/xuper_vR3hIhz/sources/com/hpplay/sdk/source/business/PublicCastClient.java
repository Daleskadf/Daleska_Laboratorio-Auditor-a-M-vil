...
public class PublicCastClient {
    ...
    private void playControl(OutParameter outParameter, final int i10, int i11, int i12, String str, int i13) {
        ...
        try {
            ...
            JSONObject genPostContent = genPostContent(stringBuffer.toString(), uid);
            ...
            if (parseInt != 0) {
                if (parseInt != 1) {
                    if (parseInt != 2) {
                        if (parseInt != 3) {
                            if (parseInt == 4) {
                                String optString3 = jSONObject.optString("er");
                                SourceLog.i(TAG, "resolvePlayStateMsg PLAY_CALLBACK_ERROR:" + optString3);
                                return;
                            }
                            return;
                    ...
                    if (onPlayStateListener != null) {
                        onPlayStateListener.onPause(optString);
                        return;
                    }
                    return;
                }
                ...
                if (onPlayStateListener2 != null) {
                    onPlayStateListener2.onStart(optString);
                }
            } else if (this.mPlayStateListener != null) {
    ...
    public void reveiveSinkServiceMsg(int i10, String str) {
        ...
        for (AbsIMMsgReceiveListener absIMMsgReceiveListener : this.mIMMsgReceiveListenerList) {
            absIMMsgReceiveListener.onMsgReceive(i10, str);
        }
    }
    ...
    public void disconnectTV(String str, String str2) {
        ...
        try {
            ...
            stringBuffer.append(",");
            stringBuffer.append(jSONObject.toString());
            JSONObject genPostContent = genPostContent(stringBuffer.toString(), str2);
            AsyncHttpParameter asyncHttpParameter = new AsyncHttpParameter(CloudAPI.sGLSBRoot + PUSH_ACTION, genPostContent.toString());
            AsyncHttpParameter.In in = asyncHttpParameter.in;
            in.requestMethod = 1;
            in.connectTimeout = 2000;
