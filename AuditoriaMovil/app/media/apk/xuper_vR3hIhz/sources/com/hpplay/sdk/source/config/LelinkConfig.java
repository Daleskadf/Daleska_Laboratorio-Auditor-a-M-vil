...
public class LelinkConfig {
    ...
    public static boolean checkBrowserBlePermission(Context context) {
        if (ContextCompat.checkSelfPermission(context, "android.permission.BLUETOOTH") == 0 && ContextCompat.checkSelfPermission(context, "android.permission.BLUETOOTH_ADMIN") == 0 && ContextCompat.checkSelfPermission(context, "android.permission.ACCESS_COARSE_LOCATION") == 0) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isLicenseMode() {
        if (Preference.getInstance().get(Preference.KEY_PERMISSION_MODE, 1) == 1) {
            return true;
        }
        return false;
    }
