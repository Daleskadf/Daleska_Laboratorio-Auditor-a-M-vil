...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T apply(BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().apply(baseRequestOptions);
        }
    ...
    public T diskCacheStrategy(DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    public T priority(Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().priority(priority);
        }
    ...
    public T removeOption(Option<?> option) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().removeOption(option);
        }
    ...
    public <Y> T set(Option<Y> option, Y y10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().set(option, y10);
        }
    ...
    public T signature(Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().signature(key);
        }
    ...
    public final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    public T transform(Transformation<Bitmap> transformation, boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().transform(transformation, z10);
        }
    ...
    public <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z10) {
        if (this.isAutoCloneEnabled) {
            return (T) mo28clone().transform(cls, transformation, z10);
        }
        Preconditions.checkNotNull(cls);
        Preconditions.checkNotNull(transformation);
        this.transformations.put(cls, transformation);
        this.isTransformationAllowed = true;
        int i10 = this.fields | 2048 | 65536;
        this.fields = i10;
