...
public final class ImmutableNetwork<N, E> extends StandardNetwork<N, E> {
    ...
    private static <N, E> NetworkConnections<N, E> connectionsOf(Network<N, E> network, N n10) {
        if (network.isDirected()) {
            Map asMap = Maps.asMap(network.inEdges(n10), sourceNodeFn(network));
            Map asMap2 = Maps.asMap(network.outEdges(n10), targetNodeFn(network));
        ...
        Map asMap3 = Maps.asMap(network.incidentEdges(n10), adjacentNodeFn(network, n10));
        if (network.allowsParallelEdges()) {
