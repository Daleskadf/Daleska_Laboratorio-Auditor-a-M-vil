...
public final class StandardMutableValueGraph<N, V> extends StandardValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public boolean removeNode(N n10) {
        ...
        for (N n11 : graphConnections.successors()) {
            GraphConnections<N, V> withoutCaching = this.nodeConnections.getWithoutCaching(n11);
            Objects.requireNonNull(withoutCaching);
            withoutCaching.removePredecessor(n10);
            this.edgeCount--;
        }
        if (isDirected()) {
            for (N n12 : graphConnections.predecessors()) {
                GraphConnections<N, V> withoutCaching2 = this.nodeConnections.getWithoutCaching(n12);
                Objects.requireNonNull(withoutCaching2);
                if (withoutCaching2.removeSuccessor(n10) != null) {
                    z10 = true;
                } else {
                    z10 = false;
                }
