...
public class VerticalViewGroupMeasure {
    ...
    public void allocateSpace(int i10) {
        ...
        for (ViewMeasure viewMeasure2 : arrayList) {
            i11 += viewMeasure2.getDesiredHeight();
        }
        int size = arrayList.size();
        if (size < 6) {
            float f11 = 1.0f - ((size - 1) * 0.2f);
            Logging.logdPair("VVGM (minFrac, maxFrac)", 0.2f, f11);
            float f12 = 0.0f;
            for (ViewMeasure viewMeasure3 : arrayList) {
                float desiredHeight = viewMeasure3.getDesiredHeight() / i11;
                ...
                viewMeasure3.setMaxDimens(this.f7039w, (int) (f10 * i10));
            }
            return;
        }
