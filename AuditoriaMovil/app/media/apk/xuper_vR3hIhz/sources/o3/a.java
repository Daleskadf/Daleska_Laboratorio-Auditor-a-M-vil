package o3;

import java.util.Set;
/* loaded from: classes.dex */
public class a extends n3.d {

    /* renamed from: v  reason: collision with root package name */
    public final n3.d f17476v;

    /* renamed from: w  reason: collision with root package name */
    public final n3.t[] f17477w;

    /* renamed from: x  reason: collision with root package name */
    public final r3.j f17478x;

    /* renamed from: y  reason: collision with root package name */
    public final k3.j f17479y;

    public a(n3.d dVar, k3.j jVar, n3.t[] tVarArr, r3.j jVar2) {
        super(dVar);
        this.f17476v = dVar;
        this.f17479y = jVar;
        this.f17477w = tVarArr;
        this.f17478x = jVar2;
    }

    @Override // n3.d
    public n3.d G(c cVar) {
        return new a(this.f17476v.G(cVar), this.f17479y, this.f17477w, this.f17478x);
    }

    @Override // n3.d
    public n3.d H(Set set, Set set2) {
        return new a(this.f17476v.H(set, set2), this.f17479y, this.f17477w, this.f17478x);
    }

    @Override // n3.d
    public n3.d I(boolean z10) {
        return new a(this.f17476v.I(z10), this.f17479y, this.f17477w, this.f17478x);
    }

    @Override // n3.d
    public n3.d J(s sVar) {
        return new a(this.f17476v.J(sVar), this.f17479y, this.f17477w, this.f17478x);
    }

    public Object M(c3.k kVar, k3.g gVar) {
        return gVar.d0(getValueType(gVar), kVar.n(), kVar, "Cannot deserialize a POJO (of type %s) from non-Array representation (token: %s): type/property designed to be serialized as JSON Array", this.f17190a.q().getName(), kVar.n());
    }

    public Object N(c3.k kVar, k3.g gVar) {
        Class cls;
        if (this.f17196g) {
            return u(kVar, gVar);
        }
        Object x10 = this.f17192c.x(gVar);
        if (this.f17199j != null) {
            E(gVar, x10);
        }
        if (this.f17204o) {
            cls = gVar.J();
        } else {
            cls = null;
        }
        n3.t[] tVarArr = this.f17477w;
        int length = tVarArr.length;
        int i10 = 0;
        while (true) {
            c3.n s02 = kVar.s0();
            c3.n nVar = c3.n.END_ARRAY;
            if (s02 == nVar) {
                return x10;
            }
            if (i10 == length) {
                if (!this.f17203n && gVar.n0(k3.h.FAIL_ON_UNKNOWN_PROPERTIES)) {
                    gVar.G0(this, nVar, "Unexpected JSON value(s); expected at most %d properties (in JSON Array)", Integer.valueOf(length));
                }
                while (kVar.s0() != c3.n.END_ARRAY) {
                    kVar.D0();
                }
                return x10;
            }
            n3.t tVar = tVarArr[i10];
            i10++;
            if (tVar != null && (cls == null || tVar.H(cls))) {
                try {
                    tVar.m(kVar, gVar, x10);
                } catch (Exception e10) {
                    K(e10, x10, tVar.getName(), gVar);
                }
            } else {
                kVar.D0();
            }
        }
    }

    public final Object O(k3.g gVar, Object obj) {
        try {
            return this.f17478x.m().invoke(obj, null);
        } catch (Exception e10) {
            return L(e10, gVar);
        }
    }

    @Override // n3.d
    public final Object c(c3.k kVar, k3.g gVar) {
        Class cls;
        n3.t tVar;
        v vVar = this.f17195f;
        y e10 = vVar.e(kVar, gVar, this.f17209t);
        n3.t[] tVarArr = this.f17477w;
        int length = tVarArr.length;
        if (this.f17204o) {
            cls = gVar.J();
        } else {
            cls = null;
        }
        Object obj = null;
        int i10 = 0;
        while (kVar.s0() != c3.n.END_ARRAY) {
            if (i10 < length) {
                tVar = tVarArr[i10];
            } else {
                tVar = null;
            }
            if (tVar == null) {
                kVar.D0();
            } else if (cls != null && !tVar.H(cls)) {
                kVar.D0();
            } else if (obj != null) {
                try {
                    obj = tVar.m(kVar, gVar, obj);
                } catch (Exception e11) {
                    K(e11, obj, tVar.getName(), gVar);
                }
            } else {
                String name = tVar.getName();
                n3.t d10 = vVar.d(name);
                if (!e10.i(name) || d10 != null) {
                    if (d10 != null) {
                        if (e10.b(d10, d10.k(kVar, gVar))) {
                            try {
                                obj = vVar.a(gVar, e10);
                                if (obj.getClass() != this.f17190a.q()) {
                                    k3.j jVar = this.f17190a;
                                    return gVar.q(jVar, String.format("Cannot support implicit polymorphic deserialization for POJOs-as-Arrays style: nominal type %s, actual type %s", d4.h.G(jVar), obj.getClass().getName()));
                                }
                            } catch (Exception e12) {
                                K(e12, this.f17190a.q(), name, gVar);
                            }
                        } else {
                            continue;
                        }
                    } else {
                        e10.e(tVar, tVar.k(kVar, gVar));
                    }
                }
            }
            i10++;
        }
        if (obj == null) {
            try {
                return vVar.a(gVar, e10);
            } catch (Exception e13) {
                return L(e13, gVar);
            }
        }
        return obj;
    }

    @Override // k3.k
    public Object deserialize(c3.k kVar, k3.g gVar) {
        if (!kVar.n0()) {
            return O(gVar, M(kVar, gVar));
        }
        if (!this.f17197h) {
            return O(gVar, N(kVar, gVar));
        }
        Object x10 = this.f17192c.x(gVar);
        n3.t[] tVarArr = this.f17477w;
        int length = tVarArr.length;
        int i10 = 0;
        while (kVar.s0() != c3.n.END_ARRAY) {
            if (i10 == length) {
                if (!this.f17203n && gVar.n0(k3.h.FAIL_ON_UNKNOWN_PROPERTIES)) {
                    gVar.w0(handledType(), "Unexpected JSON values; expected at most %d properties (in JSON Array)", Integer.valueOf(length));
                }
                while (kVar.s0() != c3.n.END_ARRAY) {
                    kVar.D0();
                }
                return O(gVar, x10);
            }
            n3.t tVar = tVarArr[i10];
            if (tVar != null) {
                try {
                    x10 = tVar.m(kVar, gVar, x10);
                } catch (Exception e10) {
                    K(e10, x10, tVar.getName(), gVar);
                }
            } else {
                kVar.D0();
            }
            i10++;
        }
        return O(gVar, x10);
    }

    @Override // n3.d
    public n3.d n() {
        return this;
    }

    @Override // n3.d
    public Object s(c3.k kVar, k3.g gVar) {
        return M(kVar, gVar);
    }

    @Override // n3.d, k3.k
    public Boolean supportsUpdate(k3.f fVar) {
        return Boolean.FALSE;
    }

    @Override // n3.d, k3.k
    public k3.k unwrappingDeserializer(d4.q qVar) {
        return this.f17476v.unwrappingDeserializer(qVar);
    }

    @Override // k3.k
    public Object deserialize(c3.k kVar, k3.g gVar, Object obj) {
        return this.f17476v.deserialize(kVar, gVar, obj);
    }
}
